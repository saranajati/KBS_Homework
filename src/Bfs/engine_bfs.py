    ...
    def _print_solutions(self):
        for i, sol in enumerate(self.solutions, 1):
            ...
            for j, (action, people, time_taken) in enumerate(sol['moves'], 1):
                action_handlers = {
                    "cross": lambda: print(f"  Step {j}: {people[0]} and {people[1]} cross → {time_taken} min"),
                    "return": lambda: print(f"  Step {j}: {people[0]} returns → {time_taken} min")
                }
                handler = action_handlers.get(action, lambda: print(
